...
2. 500MB 存储上限要求需要限定 RAW 文件策略。如果用户导入大量高清图片或多页 PDF，仅原文件就可能超过 500MB。建议采用内容寻址存储、重复文件硬链接/引用、缩略图压缩、可配置归档策略，并在 UI 显示存储预算。
...
- 使用内容寻址路径：`raw/<sha256-prefix>/<sha256>.<ext>`。
- 相同文件只存一份 RAW，多个导入记录引用同一文件。
- 为预览生成压缩缩略图，缩略图可清理重建。
- 支持用户配置存储上限和清理策略。
