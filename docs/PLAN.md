...
状态：进行中。已实现 RAW 内容寻址存储、PDF/PNG/JPG/JPEG 路径导入、SHA256/MD5 去重、`import_jobs` 写入和导入队列展示；文件选择器、拖拽导入和更完整的队列状态仍待补齐。
...
| RAW 文件超出 500MB | 存储预算失控 | 内容寻址、压缩缩略图、占用提示、清理策略 |
